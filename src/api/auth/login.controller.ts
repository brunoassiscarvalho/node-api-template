...
export default class LoginController {
  ...
  public async login(req: Request): Promise<UserCredential> {
    const {email, password}= this.decodePassAuthorization(req.headers['x-factor'] as string);
    try {
  ...
  private decodePassAuthorization(codedCredential: string) {
    ...
    if (!codedCredential) throw new UnauthorizedException("Autorização não encontrada");

    if (!codedCredential.startsWith('Basic')) throw new UnauthorizedException("Autorização básica não preenche os requisitos");
